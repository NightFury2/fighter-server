...
func main() {
	...
	ServerConn, _ := goNet.ListenUDP("udp", &goNet.UDPAddr{
		IP:   []byte{0, 0, 0, 0},
		Port: 8080,
		Zone: "",
	})
	...
	buf := make([]byte, 1024)
	fmt.Println("Connect success ", ServerConn.LocalAddr())
	...
	for {
		...
		img2, _ := gocv.NewMatFromBytes(img.Rows(), img.Cols(), img.Type(), buf)
		...
		count++
		fmt.Println("Received", " count ", count, " from ", addr)
	}
}
